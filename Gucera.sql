...
if exists(select id from   where id=@id)
set @type=0
...
begin
if(not(exists(select * from Course where name=@name)))
begin
set @success=0; ----Course Added Successfully
...
Else 
set @success=1 ----This Course in GUCera 
end
...
begin
if(exists(select * from InstructorTeachCourse where insid=@instrId and cid=@courseId))
begin
if(exists(select * from course where id=@courseId and accepted=1))
begin
set @success = 0 ---- Course Added 
update Course 
set content=@content
where id=@courseId and accepted <>0 and accepted is not null and instructorId=@instrId
end
Else
set @success = 3  ---- course have not accepted yet
end
Else
set @success = 2 --- Instructor do not teach course
end
Else 
set @success = 1 ---- Course not found
go 
----------------------------------------------------------
...
if(exists(select * from Course where id=@cid))
begin
if(exists(select * from InstructorTeachCourse where insid=@instId and cid=@cid))
begin
if(exists(select * from course where id=@cid and accepted=1))
begin
if(not(exists(select * from Assignment where number=@number and type=@type and cid=@cid)))
begin
set @success = 0  ---- Assignment Added Successfully
Insert into Assignment values (@cid ,@number,@type,@fullGrade,@weight ,@deadline ,@content)
end
Else
set @success = 4  ---- Assignment with 
end
Else
set @success = 3  ---- course have not accepted yet
end
...
if(exists(select * from Course where id=@cid))
begin
if(exists(select * from InstructorTeachCourse where insid=@instrId and cid=@cid))
begin
if(exists(select * from course where id=@cid and accepted=1))
begin
set @success = 0 ---- Successfully
...
if(exists(select * from course where id=@cid and accepted=1))
begin
if(exists(select * from Assignment where number=@assignmentNumber and type=@type and cid=@cid))
begin
if(exists(select * from StudentTakeCourse where sid=@sid and cid=@cid))
begin
if(exists(select * from StudentTakeAssignment where  cid=@cid and assignmenttype=@type and assignmentNumber=@assignmentNumber and sid=@sid))
begin
set @success = 0  ---- Assignment Graded successfully
update StudentTakeAssignment
set grade=@grade
where sid=@sid and cid=@cid and assignmentNumber=@assignmentNumber and assignmenttype =@type
end 
Else
set @success = 6  ---- Student do not submit this Assignment
end
Else
set @success = 5  ---- Student do not take this course
end
Else
set @success = 4  ---- Assignment Not found 
end
Else
set @success = 3  ---- course have not accepted yet
end
...
if(exists(select * from Course where id=@cid))
begin
if(exists(select * from InstructorTeachCourse where insid=@insId and cid=@cid))
begin
if(exists(select * from course where id=@cid and accepted=1))
begin
if(exists(select * from StudentTakeCourse where sid=@sid and cid=@cid))
...
begin
set @success = 0 ----  Certificate Issued Successfully to The student
Insert into StudentCertifyCourse values (@sid,@cid,@issueDate)
end
Else
set @success = 6 ----student already have this certificate
end
Else
set @success = 5  ---- Student do not pass this course
end
Else
set @success = 4  ---- Student do not take this course
end
Else
set @success = 3  ---- course have not accepted yet
end
...
@sid int
AS
SELECT DISTINCT Course.id , Course.name , Course.creditHours , Course.price
FROM Course where Course.id 
not in(select cid from StudentTakeCourse where StudentTakeCourse.sid=@sid)
and accepted=1
go
...
AS
BEGIN
if(exists (select * from StudentTakeCourse where sid=@sid and cid=@cid and insid=@instr))
begin
set @falg=2
end
else
